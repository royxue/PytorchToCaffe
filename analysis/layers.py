    ...
    def __init__(self, input, name=''):
        def transfer_input(_input):
            if isinstance(_input, Base):
                _input = _input()
                assert isinstance(
                    input, Blob), 'The input of layer %s is not Blob, please use nn_tools.P.blob.Blob as input' % name
            return _input
        if type(input) == list:
            # if multi input
...
class Sliding(Base):
    def __init__(self, input, kernel_size, num_out, stride=1, pad=0, name='sliding', ceil=False, transpose=False):
        ...
        for i in range(self.input.dim-2):
            if not transpose:
                if not ceil:
                    outs.append(np.floor(float(
                        self.input[2+i] + self.pad[i] * 2 - self.kernel_size[i]) / self.stride[i]) + 1)
                else:
                    outs.append(np.ceil(float(
                        self.input[2+i] + self.pad[i] * 2 - self.kernel_size[i]) / self.stride[i]) + 1)
            else:
                # transpose
                outs.append(
                    (self.input[2+i] - 1) * self.stride[i] - 2 * self.pad[i] + self.kernel_size[i])
        #     if not ceil:
...
class Conv(Sliding):
    def __init__(self, input, kernel_size, num_out, stride=1, pad=0,
        ...
        self.weight_size = np.prod(self.kernel_size) * \
            num_out * self.in_channel
        if group_size != 1:
    ...
    def __init__(self, inputs, type='sum', name='eltwise'):
        ...
        if type in ['sum', 'SUM']:
            ...
            raise AttributeError(
                'the Eltwise layer type must be sum, max or product')
