...
def profiling(net, input=None):
    ...
    for i, layer in enumerate(net.net.layer):
        ...
        if len(layer.top) == 1 and len(layer.bottom) == 1:
            ...
            if layer.type == 'PReLU':
                out = Activation(
                    blob_dict[layer.bottom[0]], 'prelu', layer.name)
            ...
            if layer.type == 'Reshape':
                out = Reshape(
                    blob_dict[layer.bottom[0]], shape=layer.reshape_param.shape.dim, name=layer.name)
            if out:
                try:
            ...
            if layer.type == 'Slice':
                ...
                out = Slice(blob_dict[layer.bottom[0]], name=layer.name,
                            slice_point=param.slice_point, axis=param.axis)
            if out:
                try:
                    not_ref.remove(blob_dict[layer.bottom[0]])
                except Exception:
