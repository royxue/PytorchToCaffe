...
class RandomSaturation(object):
    ...
    def __call__(self, image, *args):
        if random.randint(2):
            image[:, :, 1] = (
                image[:, :, 1]*random.uniform(self.lower, self.upper)).astype(np.uint8)
        if len(args):
            return (image, *args)
        else:
            return image
...
class RandomNoise(object):
    ...
    def __call__(self, image, *args):
        if np.random.randint(2):
            noise = np.random.randint(-self.max_noise,
                                      self.max_noise, image.shape)
    ...
    def __call__(self, image, *args):
        ...
        if self.type == 'constant':
            image = cv2.copyMakeBorder(
                image, size, size, size, size, cv2.BORDER_CONSTANT, value=self.constant_color)
        elif self.type == 'reflect':
            image = cv2.copyMakeBorder(
                image, size, size, size, size, cv2.BORDER_REFLECT)
